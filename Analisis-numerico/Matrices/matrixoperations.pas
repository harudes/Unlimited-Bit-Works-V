...
TMatrixOperation = Class
  ...
  public
    ...
    procedure AddRow(n:integer;m:integer);
    procedure MultiplyRow(n:integer;x:real);
...
procedure TMatrixOperation.AddRow(n:integer;m:integer);
var
  i:integer;
begin
  for i:=0 to Length(Matrix1)-1 do
  begin
    Matrix1[i,n]:=Matrix1[i,n]+Matrix1[i,m];
  end;
end;

procedure TMatrixOperation.MultiplyRow(n:integer;x:real);
var
  i:integer;
begin
  for i:=0 to Length(Matrix1)-1 do
  begin
    Matrix1[i,n]:=Matrix1[i,n]*x;
  end
end;

procedure TMatrixOperation.Triangulation();
var
  aux:Real;
...
begin
  ...
  rows:=Length(Matrix1[0]);
  SetLength(MatrixR,columns,rows);
  MultiplyRow(0,1/Matrix1[0,0]);
  for i:=1 to Length(Matrix1)-1 do
  begin
    for j:=i to Length(Matrix1)-1 do
    begin
        aux:=Matrix1[i-1,j]/Matrix1[i-1,i-1];
        MultiplyRow(i-1,aux*-1);
        AddRow(j,i-1);
        MultiplyRow(i-1,1/aux*-1);
    end;
    MultiplyRow(i,1/Matrix1[i,i]);
  end;
  MatrixR:=Matrix1;
end;
